...
function resolve(relative, vinyl, wwwroot) {
  ...
  // debug
  debug('base path: %s', gutil.colors.magenta(gutil.pathFromCwd(base)));
  debug('resolve path: %s', gutil.colors.magenta(gutil.pathFromCwd(path)));
...
function transportCssDeps(vinyl, options) {
  ...
  vinyl.contents = new Buffer(css(vinyl.contents, function(id) {
    ...
    if (!gutil.isLocal(id)) {
      ...
      // debug
      debug('module deps: %s', gutil.colors.magenta(id));
