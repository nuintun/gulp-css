 ...
 */
function print() {
  const message = gutil.apply(utils.format, null, gutil.slice.call(arguments));
...
function transportCssDeps(vinyl, options) {
  ...
  // Init css settings
  onpath = gutil.isFunction(onpath) ? (path, property) => {
  ...
  vinyl.contents = new Buffer(css(vinyl.contents, (id) => {
    if (gutil.isLocal(id)) {
      const src = id;
